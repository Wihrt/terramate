...
- `disable_changelog` stays `true` — `cog bump` does not touch `CHANGELOG.md`. This repo's
  existing `CHANGELOG.md` is hand-maintained "Keep a Changelog" format, never generated by
  cocogitto, and lacks the `- - -` marker cocogitto requires to splice in new entries.
  Flipping `disable_changelog` to `false` was tried and reverted: end-to-end sandbox testing
  (not assumption) showed every `cog bump --auto ...` invocation then fails outright with
  `Error: cannot find default separator '- - -' in CHANGELOG.md`, before creating any commit
  or tag. Leaving it `true` keeps `cog bump` working exactly as before.
- Add a `[changelog]` section pinning `template = "full_hash"` — cocogitto's built-in template
  ...
  default, so `cog changelog --at <tag>` needs no extra flags in CI. This is what actually
  produces the GitHub-Release-ready notes: `cog changelog --at <tag>` only reads git history
  and renders to stdout, so it never touches `CHANGELOG.md` and is unaffected by
  `disable_changelog`. Verified independently with both `disable_changelog` settings.
